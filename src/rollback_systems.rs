...
use crate::{game::{FrameCount, Input, Player, INPUT_UP, INPUT_DOWN, INPUT_LEFT, INPUT_RIGHT, ARENA_SIZE, INPUT_SPACE, RoundEntity}, TIME_STEP, BASE_SPEED, components::{AngularVelocity, Movable, ThrustEngine, Velocity}, LASER_SCALE, ImageAssets};
...
pub fn player_fire_system(
    ...
    mut query: Query<(&Transform, &Player), With<Rollback>>,
) {
     for (player_tf, player) in query.iter_mut() {
        let input = match inputs[player.handle].1 {
            InputStatus::Confirmed => inputs[player.handle].0.inp,
            InputStatus::Predicted => inputs[player.handle].0.inp,
            InputStatus::Disconnected => 0, // disconnected players do nothing
